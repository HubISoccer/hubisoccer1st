...
async function handleTicketSubmit(e) {
    ...
    if (file) {
        ...
        const fileName = `ticket_${currentCoach.id}_${Date.now()}.${fileExt}`;
    ...
    const { error: insertError } = await supabaseCoachPrive
        ...
        .insert([{
            user_id: currentCoach.user_id, // user_id = UUID de profiles
            subject,
            category,
            description,
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('🚀 Initialisation de la page support (coach)');

...
async function loadCoachProfile() {
    ...
    try {
        ...
        document.getElementById('userName').textContent = currentCoach ? `${currentCoach.first_name} ${currentCoach.last_name}` : 'Coach';
        document.getElementById('userAvatar').src = currentCoach?.avatar_url || 'img/user-default.jpg';
        console.log('✅ Profil utilisé :', currentCoach);
    } catch (err) {
...
async function loadDocuments() {
    try {
        const requiredDocs = [
            ...
            { id: 'licence_coach', name: 'Licence d\'entraîneur / Diplôme', type: 'coach_license' },
            { id: 'casier_judiciaire', name: 'Casier judiciaire (extrait n°3)', type: 'background_check' },
            { id: 'justificatif_domicile', name: 'Justificatif de domicile', type: 'address' }
        ];

        if (currentCoach?.id) {
            const { data: existingDocs, error } = await supabaseCoachPrive
...
async function uploadDocument(docId) {
    ...
    input.onchange = async (e) => {
        ...
        try {
            ...
            const filePath = `coach_docs/${fileName}`;
            ...
            if (doc.request_id) {
                const { error: updateError } = await supabaseCoachPrive
                    .from('coach_documents')
                    .update({ url: publicUrl, file_name: file.name, statut: 'pending' })
                    .eq('id', doc.request_id);
                if (updateError) throw updateError;
            } else {
                const { error: insertError } = await supabaseCoachPrive
                    .from('coach_documents')
                    .insert([{
...
async function submitLicense(e) {
    ...
    try {
        const formData = {
            ...
            licence: document.getElementById('licence').value || null,
        };
...
async function checkLicenseStatus() {
    if (!currentCoach || !currentCoach.id) return;
    try {
        const { data, error } = await supabaseCoachPrive
            .from('coach_license_requests')
            .select('*')
                ...
                if (data.status === 'approved' && data.carte_url) {
                    statusCard.innerHTML = `
                        ...
                        <div class="status-content">
                            ...
                            <p>Votre licence coach est prête. Cliquez ci-dessous pour télécharger votre carte.</p>
...
function updateCardPreview() {
    ...
    if (frontInfo) {
        frontInfo.innerHTML = `
            ...
            <p><span class="label">Licence :</span> <span class="value">${licence}</span></p>
        `;
    }
        ...
        frontFooter.innerHTML = `
            <div class="signatures">
                <div class="signature-box">
                    <span class="signature-label">Signature coach</span>
                </div>
                <div class="signature-box">
            ...
            <div class="id-number">ID: ${currentCoach?.hub_id || currentCoach?.id || '---'}</div>
        `;
    }
...
function saveFormToSession() {
    ...
    sessionStorage.setItem('coachLicenseFormData', JSON.stringify(formData));
}

function restoreFormFromSession() {
    const savedForm = sessionStorage.getItem('coachLicenseFormData');
    if (savedForm) {
        try {
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('🚀 Initialisation de la page verification coach');

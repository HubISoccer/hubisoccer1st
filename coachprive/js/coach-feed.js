...
async function loadProfile() {
    ...
    currentProfile = data;
    document.getElementById('userName').textContent = data.full_name || 'Coach';
...
async function loadUserMetadata() {
    const { data: likesData } = await supabaseCoachPrive
        .from('unified_likes')
        .select('post_id')
        .eq('user_id', currentProfile.id);
    likedPosts = new Set(likesData?.map(l => l.post_id) || []);

    const { data: savedData } = await supabaseCoachPrive
        .from('collection_items')
    ...
    const { data: hiddenData } = await supabaseCoachPrive
        .from('unified_hidden')
        .select('post_id')
        .eq('user_id', currentProfile.id);
...
async function loadPosts() {
    ...
    try {
        ...
        const { data: profilesData, error: profilesError } = await supabaseCoachPrive
            .from('profiles')
        ...
        for (const post of postsData) {
            ...
            const { count: likesCount } = await supabaseCoachPrive
                .from('unified_likes')
                .select('*', { count: 'exact', head: true })
                .eq('post_id', post.id);
            const { count: commentsCount } = await supabaseCoachPrive
                .from('unified_comments')
                .select('*', { count: 'exact', head: true })
                .eq('post_id', post.id);
            const { count: sharesCount } = await supabaseCoachPrive
                .from('unified_shares')
                .select('*', { count: 'exact', head: true })
                .eq('post_id', post.id);
        ...
        const { data: followingData } = await supabaseCoachPrive
            .from('unified_follows')
            .select('following_id')
            .eq('follower_id', currentProfile.id);
        const followingIds = followingData?.map(f => f.following_id) || [];

        const { data: collectionItems } = await supabaseCoachPrive
            .from('collection_items')
...
async function loadHiddenPosts() {
    ...
    try {
        ...
        const { data: postsData, error: postsError } = await supabaseCoachPrive
            .from('unified_posts')
            .select('*')
            .in('id', hiddenIds)
            .order('created_at', { ascending: false });
...
async function renderComment(comment, postId) {
    ...
    const { data: replies, error } = await supabaseCoachPrive
        .from('unified_comments')
        .select(`
            *,
            author:profiles!user_id (id, full_name, avatar_url, username)
...
async function loadFollowers() {
    const { data: followersData } = await supabaseCoachPrive
        .from('unified_follows')
    ...
    const { data: suggestionsData } = await supabaseCoachPrive
        .from('profiles')
    ...
    const { data: trendsData } = await supabaseCoachPrive
        .from('unified_posts')
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('🚀 Initialisation de coach-feed.js (communauté)');
    ...
    try {
        ...
        supabaseCoachPrive
            .channel('unified_posts_changes_coach')
        ...
        supabaseCoachPrive
            .channel('notifications_changes_coach')
...
function openPreview() {
    ...
    document.getElementById('previewAuthorName').textContent = currentProfile.full_name || 'Coach';

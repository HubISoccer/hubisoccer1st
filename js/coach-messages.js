...
// Copier exactement le contenu de agent-messages.js, remplacer supabaseAgentPrive par supabaseCoachPrive
...
async function loadConversations() {
    ...
    const { data: archivedData } = await supabaseAgentPrive
    ...
    const { data: blockedData } = await supabaseAgentPrive
        .from('blocked_users')
        .select('blocked_id')
    ...
    if (contactIds.length > 0) {
        const { data: presenceData } = await supabaseAgentPrive
            .from('user_presence')
...
async function findOrCreateConversationWithUser(userId) {
    ...
    const { data: existingConv, error: searchError } = await supabaseAgentPrive
        .from('conversations')
        .select('id')
    ...
    const { data: newConv, error: createError } = await supabaseAgentPrive
        .from('conversations')
        .insert([{
...
async function selectConversation(convId) {
    ...
    if (conv) {
        const { data: contactProfile } = await supabaseAgentPrive
            .from('profiles')
            .select('*')
    ...
    messagesSubscription = supabaseAgentPrive
...
async function loadMessages(convId) {
    ...
    if (replyIds.length > 0) {
        const { data: replies, error: replyError } = await supabaseAgentPrive
            .from('messages')
...
async function getSignedUrl(path, expiresIn = 3600) {
    ...
    const { data, error } = await supabaseAgentPrive.storage
        .from(bucket)
...
async function insertMessage(conversationId, content, attachmentUrl, attachmentPath) {
    ...
    const { data: newMsg, error: msgError } = await supabaseAgentPrive
        .from('messages')
        .insert(insertData)
        .select()
        .single();
...
async function archiveConversation() {
    ...
    const { data: existing, error: checkError } = await supabaseAgentPrive
        .from('archived_conversations')
        .select('id')
        .eq('user_id', currentProfile.id)
...
async function confirmDeleteConversation() {
    ...
    const { error: msgError } = await supabaseAgentPrive
        .from('messages')
        .delete()
        .eq('conversation_id', currentConversationId);
    if (msgError) {
    ...
    const { error: convError } = await supabaseAgentPrive
        .from('conversations')
        .delete()
...
async function loadBlockedUsers() {
    ...
    const { data: profiles, error: profilesError } = await supabaseAgentPrive
        .from('profiles')
...
async function ensureSupportConversation() {
    ...
    const { data: supportData, error: searchError } = await supabaseAgentPrive
        .from('profiles')
        .select('id')
        .eq('username', 'SUPPORT')
        .maybeSingle();
    ...
    if (!supportData) {
        ...
        const { data: newSupport, error: insertError } = await supabaseAgentPrive
            .from('profiles')
            .insert([{
    ...
    const { data: existingConv, error: convCheckError } = await supabaseAgentPrive
        .from('conversations')
        .select('id')
    ...
    const { data: newConv, error: convError } = await supabaseAgentPrive
        .from('conversations')
        .insert([{
            participant1_id: currentProfile.id,
...
function subscribeToNewConversations() {
    conversationsSubscription = supabaseAgentPrive
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('🚀 Initialisation messages (agent)');

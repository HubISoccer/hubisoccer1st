...
async function loadUserMetadata() {
    // Likes
    const { data: likesData } = await supabaseAgentPrive
        .from('unified_likes')
        .select('post_id')
        .eq('user_id', currentProfile.id);
    ...
    const { data: savedData } = await supabaseAgentPrive
        .from('collection_items')
    ...
    // Hidden
    const { data: hiddenData } = await supabaseAgentPrive
        .from('unified_hidden')
        .select('post_id')
        .eq('user_id', currentProfile.id);
...
async function loadPosts() {
    ...
    try {
        ...
        const { data: profilesData, error: profilesError } = await supabaseAgentPrive
            .from('profiles')
        ...
        for (const post of postsData) {
            ...
            const { count: likesCount } = await supabaseAgentPrive
                .from('unified_likes')
                .select('*', { count: 'exact', head: true })
                .eq('post_id', post.id);
            const { count: commentsCount } = await supabaseAgentPrive
                .from('unified_comments')
                .select('*', { count: 'exact', head: true })
                .eq('post_id', post.id);
            const { count: sharesCount } = await supabaseAgentPrive
                .from('unified_shares')
                .select('*', { count: 'exact', head: true })
                .eq('post_id', post.id);
        ...
        const { data: followingData } = await supabaseAgentPrive
            .from('unified_follows')
            .select('following_id')
            .eq('follower_id', currentProfile.id);
        const followingIds = followingData?.map(f => f.following_id) || [];

        const { data: collectionItems } = await supabaseAgentPrive
            .from('collection_items')
...
async function loadHiddenPosts() {
    ...
    try {
        ...
        const { data: postsData, error: postsError } = await supabaseAgentPrive
            .from('unified_posts')
            .select('*')
            .in('id', hiddenIds)
            .order('created_at', { ascending: false });
...
async function renderComment(comment, postId) {
    ...
    const { data: replies, error } = await supabaseAgentPrive
        .from('unified_comments')
        .select(`
            *,
            author:profiles!user_id (id, full_name, avatar_url, username)
...
async function loadFollowers() {
    const { data: followersData } = await supabaseAgentPrive
        .from('unified_follows')
    ...
    const { data: suggestionsData } = await supabaseAgentPrive
        .from('profiles')
    ...
    const { data: trendsData } = await supabaseAgentPrive
        .from('unified_posts')
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('🚀 Initialisation de agent-feed.js (communauté)');
    ...
    try {
        ...
        supabaseAgentPrive
            .channel('unified_posts_changes_agent')
        ...
        supabaseAgentPrive
            .channel('notifications_changes_agent')
...
function openPreview() {
    ...
    document.getElementById('previewAuthorName').textContent = currentProfile.full_name || 'Agent';

...
async function loadTournamentDetails(tournamentId) {
    try {
        const { data: playersData, error } = await supabaseAgentPrive
...
function renderChatMessages() {
    ...
    chatDiv.innerHTML = messages.map(msg => {
        const isOwn = msg.userId === currentAgent.id;
        return `
...
function subscribeToMessages(tournamentId) {
    ...
    messagesSubscription = supabaseAgentPrive
        ...
        .on('postgres_changes', {
            ...
            if (!authorName) {
                const { data } = await supabaseAgentPrive
                    .from('profiles')
                    .select('full_name')
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('🚀 Initialisation de la page tournois (agent)');

...
async function uploadFile(file) {
    ...
    const fileName = `${currentAgent.id}_${Date.now()}.${fileExt}`;
    const filePath = `agent_certifications/${fileName}`;

    const { error } = await supabaseAgentPrive.storage
        .from('documents')
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('🚀 Initialisation de la page certifications (agent)');

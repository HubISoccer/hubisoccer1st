...
async function loadJoueurs(search = '') {
    ...
    if (search) query = query.or(`nom.ilike.%${search}%,description.ilike.%${search}%,region.ilike.%${search}%`);
...
async function loadDons(search = '') {
    ...
    if (search) query = query.or(`titre.ilike.%${search}%,description.ilike.%${search}%,region.ilike.%${search}%`);
    const { data, error } = await query;
    if (error) console.error(error);
...
async function loadTemoignages(search = '') {
    ...
    if (search) query = query.or(`auteur.ilike.%${search}%,texte.ilike.%${search}%`);
    const { data, error } = await query;
    if (error) console.error(error);
...
async function loadMessages(search = '') {
    ...
    const { data, error } = await query;
    if (error) console.error(error);
    else { currentMessages = data || []; renderMessages(currentMessages); }
}
...
function renderJoueurs(joueurs) {
    if (!joueurs.length) { joueursList.innerHTML = '<p class="no-data">Aucun joueur.</p>'; return; }
    let html = '';
        ...
        html += `
            <div class="list-item" data-id="${item.id}">
                <div class="info">
                    ...
                    <div class="details">
                        <span>${item.poste}</span><span>${item.region}</span><span>${item.besoin}</span><span>${item.montant}</span>
                    </div>
...
function renderDons(dons) {
    if (!dons.length) { donsList.innerHTML = '<p class="no-data">Aucun appel aux dons.</p>'; return; }
    let html = '';
    dons.forEach(item => {
        html += `
            <div class="list-item" data-id="${item.id}">
                <div class="info">
                    ...
                    <div class="details">
                        <span>${item.region}</span><span>Objectif: ${item.objectif}</span><span>Collecté: ${item.collecte}</span>
                    </div>
                    <small>${(item.description || '').substring(0, 80)}...</small>
                </div>
                <div class="actions">
...
function renderTemoignages(temoignages) {
    if (!temoignages.length) { temoignagesList.innerHTML = '<p class="no-data">Aucun témoignage.</p>'; return; }
    let html = '';
...
function renderMessages(messages) {
    if (!messages.length) { messagesList.innerHTML = '<p class="no-data">Aucun message.</p>'; return; }
    let html = '';
        ...
        html += `
            <div class="list-item ${!msg.is_read ? 'unread' : ''}" data-id="${msg.id}">
                <div class="info">
                    ...
                    <div class="details">
                        <span>Type: ${msg.type}</span><span>Cible: ${msg.target_title || msg.target_id || '-'}</span><span>${new Date(msg.created_at).toLocaleString()}</span>
                    </div>
...
window.openModal = (type, id = null) => {
    ...
    if (type === 'joueur') {
        ...
        fields = `
            ...
            <div class="form-group"><label>Média (image/vidéo)</label><input type="file" id="imageFile" accept="image/*,video/*"></div>
        ...
        fields = `
            ...
            <div class="form-group"><label>Avatar (image)</label><input type="file" id="avatarFile" accept="image/*"></div>
...
window.deleteItem = async (type, id) => {
    ...
    else { showToast('Supprimé', 'success'); loadAll(); }
};
...
window.deleteMessage = async (id) => {
    ...
    if (error) showToast('Erreur : ' + error.message, 'error');
    else { showToast('Message supprimé', 'success'); loadMessages(); }
};

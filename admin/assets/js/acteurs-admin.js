(() => {
        ...
        container.innerHTML = list.map(c => {
            ...
            return `
                <div class="list-item" data-id="${c.id}">
                    ...
                    <div class="actions">
                        <button class="view" onclick="window.viewCandidature('${c.id}')" title="Voir détails"><i class="fas fa-eye"></i></button>
                        <button class="delete" onclick="window.deleteCandidature('${c.id}')" title="Supprimer"><i class="fas fa-trash"></i></button>
                    </div>
                </div>
            `;
        }).join('');
    ...
    function renderSportifs(list) {
        ...
        container.innerHTML = list.map(s => `
            <div class="list-item" data-id="${s.id}">
                ...
                <div class="actions">
                    <button class="edit" onclick="window.editSportif('${s.id}')" title="Modifier"><i class="fas fa-edit"></i></button>
                    <button class="delete" onclick="window.deleteSportif('${s.id}')" title="Supprimer"><i class="fas fa-trash"></i></button>
                </div>
            </div>
        `).join('');
    }
    ...
    function renderDons(list) {
        ...
        container.innerHTML = list.map(d => `
            <div class="list-item" data-id="${d.id}">
                ...
                <div class="actions">
                    <button class="edit" onclick="window.editDon('${d.id}')" title="Modifier"><i class="fas fa-edit"></i></button>
                    <button class="delete" onclick="window.deleteDon('${d.id}')" title="Supprimer"><i class="fas fa-trash"></i></button>
                </div>
            </div>
        `).join('');
    }
    ...
    async function loadTemoignages() {
        ...
        try {
            ...
            renderTemoignages(temoignagesList);
        } catch (err) {
            console.error(err);
    ...
    function renderTemoignages(list) {
        ...
        container.innerHTML = list.map(t => `
            <div class="list-item" data-id="${t.id}">
                ...
                <div class="actions">
                    <button class="edit" onclick="window.editTemoignage('${t.id}')" title="Modifier"><i class="fas fa-edit"></i></button>
                    <button class="delete" onclick="window.deleteTemoignage('${t.id}')" title="Supprimer"><i class="fas fa-trash"></i></button>
                </div>
            </div>
        `).join('');
    }
    ...
    // Boutons d'action sur la candidature
    document.getElementById('validCandidatureBtn').addEventListener('click', () => {
        if (currentCandidature) window.updateCandidatureStatus(currentCandidature, 'approved');
    });
    document.getElementById('pauseCandidatureBtn').addEventListener('click', () => {
        if (currentCandidature) window.updateCandidatureStatus(currentCandidature, 'suspended');
    });
    document.getElementById('rejectCandidatureBtn').addEventListener('click', () => {
        if (currentCandidature) window.updateCandidatureStatus(currentCandidature, 'rejected');
    });
    document.getElementById('deleteCandidatureBtn').addEventListener('click', () => {
        if (currentCandidature) window.deleteCandidature(currentCandidature);
    });
    ...
    window.closeSportifModal = closeSportifModal;
    window.closeDonModal = closeDonModal;
    window.closeTemoignageModal = closeTemoignageModal;

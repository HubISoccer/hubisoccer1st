...
const allCountries = [
    ...
    "Guinée équatoriale", "Guyana", "Haïti", "Honduras", "Hongrie", "Inde", "Indonésie", "Irak", "Iran", "Irlande", "Islande", "Israël",
    "Italie", "Jamaïque", "Japon", "Jordanie", "Kazakhstan", "Kenya", "Kirghizistan", "Kiribati", "Koweït", "Laos", "Lesotho", "Lettonie",
    "Liban", "Libéria", "Libye", "Liechtenstein", "Lituanie", "Luxembourg", "Macédoine du Nord", "Madagascar", "Malaisie", "Malawi",
    "Maldives", "Mali", "Malte", "Maroc", "Marshall", "Maurice", "Mauritanie", "Mexique", "Micronésie", "Moldavie", "Monaco", "Mongolie",
    "Monténégro", "Mozambique", "Namibie", "Nauru", "Népal", "Nicaragua", "Niger", "Nigeria", "Norvège", "Nouvelle-Zélande", "Oman",
    "Ouganda", "Ouzbékistan", "Pakistan", "Palaos", "Panama", "Papouasie-Nouvelle-Guinée", "Paraguay", "Pays-Bas", "Pérou", "Philippines",
    "Pologne", "Portugal", "Qatar", "République centrafricaine", "République dominicaine", "République tchèque", "Roumanie", "Royaume-Uni",
    "Russie", "Rwanda", "Saint-Christophe-et-Niévès", "Sainte-Lucie", "Saint-Marin", "Saint-Vincent-et-les-Grenadines", "Salomon", "Salvador",
    "Samoa", "São Tomé-et-Príncipe", "Sénégal", "Serbie", "Seychelles", "Sierra Leone", "Singapour", "Slovaquie", "Slovénie", "Somalie",
    "Soudan", "Soudan du Sud", "Sri Lanka", "Suède", "Suisse", "Suriname", "Syrie", "Tadjikistan", "Tanzanie", "Tchad", "Thaïlande",
...
// ==================== RELATIONS (ABONNÉS / ABONNEMENTS / SUGGESTIONS / TENDANCES) ====================
async function loadFollowers() {
    ...
    if (followersList) {
        if (!followers.length) {
            followersList.innerHTML = '<li>Aucun abonné</li>';
        } else {
            followersList.innerHTML = followers.slice(0, 5).map(f => `
                <li onclick="openUserProfile('${f.follower_id}')">
                    <img src="${f.profiles?.avatar_url || 'img/user-default.jpg'}">
                    <span>${escapeHtml(f.profiles?.full_name || 'Anonyme')}</span>
                    <small>@${f.profiles?.username || ''}</small>
                </li>
            `).join('');
        }
    }
...
async function loadFollowing() {
    ...
    if (followingList) {
        if (!following.length) {
            followingList.innerHTML = '<li>Aucun abonnement</li>';
        } else {
...
async function loadSuggestions() {
    ...
    if (suggestionsList) {
        if (!data.length) {
            ...
            suggestionsList.innerHTML = data.map(s => `
...
async function loadTrends() {
    ...
    if (trendsList) {
        if (!sorted.length) {
            trendsList.innerHTML = '<li>Aucune tendance</li>';
        } else {
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div id="storyUploadProgress" style="display: none; margin-top: 10px;">
                    ...
                    <p id="storyUploadPercent" style="font-size: 0.8rem; margin-top: 5px;">0%</p>
                </div>
                <button class="btn-create-story" id="publishStoryBtn" onclick="uploadStory()">Publier la story</button>
            </div>
        </div>
    `;
...
// ==================== PROFIL UTILISATEUR (version finale, avec bouton Suivre) ====================
async function openUserProfile(userId) {
    ...
    // Ajouter les compteurs (s’ils n’existent pas déjà)
...
async function toggleFollow(button) {
    ...
    withButtonSpinner(button, async () => {
        ...
        // Mettre à jour les boutons dans les posts
        renderPosts();
    });
}
...
async function loadLives() {
    ...
    try {
        ...
        const livesWithProfiles = data.map(live => ({
            ...live,
            profiles: profiles.find(p => p.id === live.user_id)
        }));
        renderLivesList(livesWithProfiles);
    } catch (err) {
